...
### 2.4 实践洞察：协调者、专家与 Subagent 的职责边界

从真实场景可以更直观地理解三层角色各自存在的理由。

**为什么需要协调者？**  
当一件大事依赖**多个专家 Agent** 时，需要有人负责「谁先谁后、何时切换、如何衔接」。专家只擅长自己那一类事（文件、翻译、写代码…）；协调者则掌握「要完成整件大事需要哪几个专家、按什么顺序调、中间结果怎么传给下一个」。用户只提一个总目标，由协调者负责在多个专家之间**协调与切换**。

**场景示例：参考另一仓库写代码**

假设任务为「参考仓库 A 的代码，修改当前项目」。过程会经历：

1. **多次查找**：在参考仓库里搜索相关实现、读文件、可能多次试错。
2. **只有和主线相关的内容才被主线使用**：大量搜索结果中，只有与当前需求相关的片段会参与后续的「写代码/改代码」决策。
3. **再继续找更多**：根据当前进度，可能再发起新一轮检索。

若采用**两层结构**（例如只有一个 Coding Agent + 一个检索 Agent），且把检索 Agent 的每一轮尝试（无论找到与否）都原样塞进 Coding Agent 的上下文，会产生：

- **大量 token**：检索过程中的所有搜索、失败尝试、无关结果都进入主线。
- **错误上下文干扰**：主线 Agent 容易被「没找到」「试错了」等噪音干扰，表现下降。

因此更合理的形态是：

| 层级 | 角色 | 职责 |
| :--- | :--- | :--- |
| **协调者** | Manager / 协调者 | 路由与协调。拆任务、决定「谁来做」、汇总结果；不直接执行具体领域任务。 |
| **专家 Agent** | 领域专家 | 具备领域心智（如写代码、翻译、写报告），只消费「该看的」上下文，完成领域内的决策与产出。 |
| **Subagent** | 工具型能力 | 检索、搜索、更新记忆等偏功能性；拥有**独立 Agent 循环**，多轮尝试在内部完成，**只把对主线有用的、筛选后的结果**向上输出，避免将大量尝试过程暴露给专家或协调者。 |

在上述「参考仓库写代码」场景中：协调者拆成「先取参考、再改代码」；检索 Subagent 在自己的循环里多轮搜索并过滤，只返回与主线相关的片段/摘要；Coding 专家只收到「筛选后的参考内容」再执行修改。这样既满足多轮查找、按需再找的流程，又避免两层结构下的上下文污染。
