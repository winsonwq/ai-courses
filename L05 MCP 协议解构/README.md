...
本课程提供了丰富的可视化图表，帮助理解 MCP 协议。所有图表都在 `ARCHITECTURE.md` 文件中，包括：

### 🔗 连接时序图（重点推荐）

**连接时序图是理解 MCP 协议的关键**，详细展示了连接建立、断开、重连的完整过程：

1. **连接建立完整时序图（Stdio 传输）** ⭐ - 从进程启动到就绪的完整流程
2. **连接建立完整时序图（SSE 传输）** ⭐ - HTTP/SSE 连接的建立过程
3. **连接断开和清理时序图** ⭐ - 正常断开、异常断开、网络断开的处理
4. **重连机制时序图** ⭐ - 自动重连的实现策略
5. **多 Server 并发连接时序图** ⭐ - 同时连接多个 Server 的时序
6. **握手流程（简化版）** - 握手阶段的核心步骤

### 📐 其他重要图表

7. **集成灾难问题示意图** - 展示为什么需要 MCP（N×M 问题）
8. **USB 协议类比图** - 帮助理解 MCP 的设计哲学
9. **核心架构图** - Host、Client、Server 三者的关系
10. **协议栈分层图** - 从应用层到传输层的完整分层
11. **工具发现流程** - 工具如何被发现和注入 LLM
12. **工具执行流程** - 完整的工具调用过程
13. **完整通信流程** - 端到端的通信示例
14. **生命周期状态图** - Client 和 Server 的状态转换
15. **多 Server 连接架构** - 一个 Host 连接多个 Server
16. **数据流图** - 数据在系统中的完整流动
17. **错误处理流程** - 错误传播和处理机制
18. **工具 Schema 转换流程** - MCP 格式到 Function Calling 格式
19. **Stdio 传输详细流程** - 本地进程通信的实现细节
20. **SSE 传输详细流程** - 远程 HTTP 通信的实现细节
21. **MCP vs Function Calling 对比** - 概念辨析
22. **Stdio vs SSE 传输对比** - 两种传输方式对比
23. **JSON-RPC 消息格式** - 消息结构说明

> 💡 **学习建议**：
> - **首先学习连接时序图**（第 2-2.5 节），这是理解 MCP 协议的基础
